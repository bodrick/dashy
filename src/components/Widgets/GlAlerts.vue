<template>
  <div v-if="alerts" class="glances-alerts-wrapper">
    <div v-for="(alert, index) in alerts" :key="index" class="alert-row">
      <p v-tooltip="tooltip(`${alert.timeAgo}<br>Lasted: ${alert.lasted}`, true)" class="time">
        {{ alert.time }}
        <span v-if="alert.ongoing" class="ongoing">Ongoing</span>
      </p>
      <div v-tooltip="tooltip(alert.minMax, true)" class="alert-info">
