  ...
  <div class="stat-ping-wrapper">
    ...
    >
      ...
      <p class="service-name">
        ...
        <Button class="far fa-info" @click="service.infoHidden = !service.infoHidden"></Button>
        <Button
          class="far fa-chart-line"
          @click="service.chartHidden = !service.chartHidden"
        ></Button>
      </p>
      <!-- Charts -->
      <div v-if="!service.chartHidden" class="charts">
        <img
          class="uptime-pie-chart"
...
export default {
  ...
  computed: {
    ...
    showChart() {
      return typeof this.options.showChart === 'boolean' ? this.options.showChart : true;
    },
    showInfo() {
      return typeof this.options.showInfo === 'boolean' ? this.options.showInfo : true;
    },
  },
  methods: {
    fetchData() {
    ...
    makeTooltip(service) {
      const { responseTime, totalFailure, totalSuccess, lastSuccess, lastFailure } = service;
      const content =
        `<b>Failed Pings:</b> ${totalFailure}/${totalSuccess}<br>` +
    ...
    processData(data) {
      ...
      for (const service of data) {
        if (this.groupId && this.groupId !== service.group_id) continue;
        services.push({
          name: service.name,
