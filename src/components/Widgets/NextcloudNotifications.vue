  ...
  <div class="nextcloud-widget nextcloud-status-wrapper">
    <div v-if="notifications.length > 0">
      ...
      <div v-for="(notification, idx) in notifications" :key="idx" class="notification">
        ...
        <div>
          <p>
            <small v-tooltip="dateTooltip(notification)" class="date">
              {{ getTimeAgo(Date.parse(notification.datetime)) }}
            </small>
            <span v-tooltip="subjectTooltip(notification)">{{ notification.subject }} </span>
            <!-- notifications item: action links -->
            <span v-if="notification.actions.length > 0">
  ...
  computed: {
    ...
    limit() {
      ...
      if (typeof lim === 'string') {
        ...
        const m = lim.match(/(\d+)([dhm])/);
        if (m.length !== 3) return defaultLimit;
        return [false, m[1] * k[m[2]] * 1000];
      }
      if (typeof lim === 'number') {
        return [Number.parseInt(this.options.limit, 10) || 0, false];
      }
      return defaultLimit;
    },
  },
  ...
  methods: {
    ...
    processNotifications(response) {
      ...
      for (const notification of notifications) {
        if (limitCount && this.notifications.length === limitCount) continue; // count limit
        const notiTime = Date.parse(notification.datetime);
        const nowTime = Date.now();
        if (limitTime && notiTime && nowTime - notiTime > limitTime) continue; // time limit
    ...
    deleteNotifications() {
      this.makeRequest(this.endpoint('notifications'), this.headers, 'DELETE').then(() => {
        this.notifications = [];
      });
    },
    deleteNotification(id) {
      this.makeRequest(`${this.endpoint('notifications')}/${id}`, this.headers, 'DELETE').then(
        this.fetchData
      );
    },
    /* Tooltip generators */
