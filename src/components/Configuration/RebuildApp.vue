  ...
  <modal :name="modalName" :resizable="true" width="50%" height="60%" classes="dashy-modal">
    <div class="rebuild-app-container">
      ...
      <Button :click="startBuild" :disabled="loading || !allowRebuild" :disallow="!allowRebuild">
        ...
        <template #icon><RebuildIcon /></template>
      </Button>
      ...
      <div v-if="success !== undefined" class="rebuild-response">
        <p v-if="success" class="response-status success">✅ {{ $t('app-rebuild.success-msg') }}</p>
        <p v-else class="response-status failure">❌ {{ $t('app-rebuild.fail-msg') }}</p>
        ...
        <Button v-if="success" :click="refreshPage">
          <template #text>{{ $t('app-rebuild.reload-button') }}</template>
...
export default {
  ...
  mounted() {
    ...
    if (
      this.appConfig.allowConfigEdit === false ||
  ...
  methods: {
    ...
    finished(responseData) {
      this.loading = false;
      if (responseData) {
        const { success, output, error, message } = responseData;
