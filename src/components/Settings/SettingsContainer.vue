<template>
  <section :class="{ 'settings-hidden': !settingsVisible }">
    <SearchBar v-if="searchVisible" ref="SearchBar" @user-is-searching="userIsTypingSomething" />
    <div class="options-outer">
      <div :class="`options-container ${!settingsVisible ? 'hide' : ''}`">
        <ThemeSelector />
        <LayoutSelector :display-layout="displayLayout" />
        <ItemSizeSelector :icon-size="iconSize" />
        <ConfigLauncher />
        <AuthButtons v-if="userState !== 0" :user-type="userState" />
      </div>
...
import { localStorageKeys, visibleComponents as defaultVisibleComponents } from '@/utils/defaults';
...
export default {
  ...
  methods: {
    ...
    userIsTypingSomething(something) {
      this.$emit('user-is-searching', something);
    },
    ...
    getSettingsVisibility() {
      ...
      if (
        ...
        (this.visibleComponents || {}).settings === false ||
        localStorage[localStorageKeys.HIDE_SETTINGS] === 'false'
      )
        return false;
