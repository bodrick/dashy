<template>
  <transition name="slide-fade">
    <div v-if="!shouldHide" class="kb-sc-info">
      ...
      <p title="Press [Esc] to hide this tip forever. See there's even a shortcut for that! 🚀">
        Just start typing to filter. Then use the tab key to cycle through results, and press enter
        to launch the selected item, or alt + enter to open in a modal. You can hit Esc at anytime
        to clear the search. Easy 🥳
      </p>
    </div>
  </transition>
</template>
...
export default {
  ...
  methods: {
     ...
     * Trigger the hide function, and remove the event listener
     */
    hideWelcomeHelper() {
...
.kb-sc-info {
  ...
  .close {
    /* The little exit icon, in top-right */
    float: right;
    border-radius: 20px;
    width: 1em;
    padding: 0 0 6px 6px;
...
.slide-fade-leave-active {
  transition: all 0.8s cubic-bezier(0.93, 0.01, 0.89, 0.5);
}
