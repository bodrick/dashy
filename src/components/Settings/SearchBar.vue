<template>
  <form :class="minimalSearch ? 'minimal' : 'normal'" @submit.prevent="searchSubmitted">
...
export default {
  ...
  data() {
    return {
      ...
      akn: new ArrowKeyNavigation(), // Class that manages arrow key navigation
  ...
  methods: {
    ...
    userIsTypingSomething() {
      this.$emit('user-is-searching', this.input);
    },
    ...
    handleHotKey(key) {
      ...
      for (const hotkey of usersHotKeys) {
        if (hotkey.hotkey === Number.parseInt(key, 10) && hotkey.url)
    ...
    searchSubmitted() {
      ...
      if (!searchPrefs.disableWebSearch) {
        ...
        // Use either search bang, or preferred search engine
